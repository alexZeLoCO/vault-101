...
2. El servidor recibe una copia de la clave pública en `~/.ssh/authorized_keys`.
3. El cliente envía una copia de su clave pública.
4. El servidor compara la clave con las que tiene para ese usuario.
5. El servidor cifra un reto con la clave pública y la envía al cliente.
6. El cliente recibe el reto, lo descifra con su clave privada.
7. El cliente envía el resultado al servidor.
8. El servidor compara lo que retorna el cliente con los datos originales.
9. Si coinciden, la autenticación es correcta.
